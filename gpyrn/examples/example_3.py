...
weight = [
    covfunc.SquaredExponential(20, 5),
    covfunc.SquaredExponential(0.1, 10),
    covfunc.SquaredExponential(10, 5),
    covfunc.SquaredExponential(1, 10),
]
means = [meanfunc.Constant(0), meanfunc.Constant(0)]
jitter = [0.5, 0.5]

elbo, m, v = gprn.ELBOcalc(
    nodes, weight, means, jitter, iterations=5000, mu="init", var="init"
)
print("ELBO =", elbo)

tstar = np.linspace(time.min(), time.max(), 1000)
a, _, _, b = gprn.newPrediction(
