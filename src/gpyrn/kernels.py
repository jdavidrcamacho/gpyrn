...
from .utils import _array_input


class CovarianceFunction:
    """Base class for covariance functions used by GPRN nodes and weights."""
    ...
    def __repr__(self):
        if hasattr(self, "_param_names"):
            pars = ", ".join([f"{p}={v}" for p, v in zip(self._param_names, self.pars)])
        else:
...
class _operator(CovarianceFunction):
...
class _unary_operator(CovarianceFunction):
...
class QuasiPeriodic(CovarianceFunction):
    r"""
    ...
    def _dkdxidj(self, r):
        ...
        term3 = np.exp(
            -(ellp**2 * r**2 + 2 * elle**2 * np.sin(np.pi * r / P) ** 2)
            / (ellp**2 * elle**2)
        )
        return term1 * term2 * term3
...
class RQP(CovarianceFunction):
    ...
    def __init__(self, theta: float, alpha: float, elle: float, P: float, ellp: float):
    ...
    def __call__(self, r):
        ...
        return (
            θ**2
            * np.exp(-2 * np.sin(np.pi * np.abs(r) / P) ** 2 / ℓp**2)
            * (1 + r**2 / (2 * α * ℓe**2)) ** (-α)
        )


class Cosine(CovarianceFunction):
    """
    the cosine kernel
...
class Matern32(CovarianceFunction):
    ...
    def __call__(self, r):
        return (
            self.pars[0] ** 2
            * (1.0 + np.sqrt(3.0) * np.abs(r) / self.pars[1])
...
class GammaExp(CovarianceFunction):
    """
...
class Piecewise(CovarianceFunction):
    """
...
class Paciorek(CovarianceFunction):
    """
...
class NewPeriodic(CovarianceFunction):
    """
    ...
    def __call__(self, r):
        a = (
            1 + 2 * np.sin(np.pi * np.abs(r) / self.P) ** 2 / (self.alpha2 * self.l**2)
        ) ** (-self.alpha2)
        return self.amplitude**2 * a


class QuasiNewPeriodic(CovarianceFunction):
    """
    ...
    def __call__(self, r):
        a = (
            1
            + 2
            * np.sin(np.pi * np.abs(r) / self.P) ** 2
            / (self.alpha2 * self.ell_p**2)
        ) ** (-self.alpha2)
        b = np.exp(-0.5 * r**2 / self.ell_e**2)
        return self.amplitude**2 * a * b


class NewRQP(CovarianceFunction):
    """
    Definition of a new quasi-periodic kernel. Derived from mapping the rational
    quadratic kernel to the 2D space u(x) = (cos x, sin x) and multiplying it by
    ...
    def __init__(self, amplitude, alpha1, alpha2, ell_e, P, ell_p):
        super(NewRQP, self).__init__(amplitude, alpha1, alpha2, ell_e, P, ell_p)
        self.amplitude = amplitude
        self.alpha1 = alpha1
        self.alpha2 = alpha2
        self.ell_e = ell_e
...
class HarmonicPeriodic(CovarianceFunction):
    """
    ...
    def __call__(self, t1, t2):
        first = (
            np.sin((self.N + 0.5) * 2 * np.pi * t1 / self.P)
            / 2
            * np.sin(np.pi * t1 / self.P)
        )
        second = (
            np.sin((self.N + 0.5) * 2 * np.pi * t2 / self.P)
            / 2
            * np.sin(np.pi * t2 / self.P)
        )
        firstPart = (first - second) ** 2
        first = 0.5 / np.tan(np.pi * t1 / self.P)
        second = (
            np.cos((self.N + 0.5) * 2 * np.pi * t1 / self.P)
            / 2
            * np.sin(np.pi * t1 / self.P)
        )
        third = 0.5 / np.tan(np.pi * t2 / self.P)
        fourth = (
            np.cos((self.N + 0.5) * 2 * np.pi * t2 / self.P)
            / 2
            * np.sin(np.pi * t2 / self.P)
        )
...
class QuasiHarmonicPeriodic(CovarianceFunction):
    """
...
class CosPeriodic(CovarianceFunction):
    """
    ...
    def __call__(self, r):
        return self.amplitude**2 * np.exp(
            -2 * np.cos(np.pi * np.abs(r) / self.P) ** 2 / self.ell**2
        )


class QuasiCosPeriodic(CovarianceFunction):
    """
    ...
    def __call__(self, r):
        return self.amplitude**2 * np.exp(
            -2 * np.cos(np.pi * np.abs(r) / self.P) ** 2 / self.ell_p**2
            - r**2 / (2 * self.ell_e**2)
        )
