...
def equal_y_axis(ax):
    """Set all axes in a sequence to the same y-axis limits."""
    ylim = ax.get_ylim()
...
def plot_prediction(gprn, nn=1000, tstar=None, over=0.2, title=None):
    ...
    a, v = gprn.predict_from_variational_parameters()
    aa, vv, bb = gprn.predict_from_variational_parameters(tstar=tstar, separate=True)
    ss = np.sqrt(vv)
    ...
    for i in range(gprn.p):
        ...
        axs[f"pred{i+1}"].errorbar(gprn.time, gprn.y[i], gprn.yerr[i], fmt="ok", ms=2)
        ...
        resid = gprn.y[i] - a[:, i]
        axs[f"resid{i+1}"].errorbar(gprn.time, resid, gprn.yerr[i], fmt="ok", ms=2)
        axs[f"resid{i+1}"].axhline(y=0.0, ls="--", color="k", alpha=0.2)
        axs[f"resid{i+1}"].set_title(
            f"std: {resid.std():.2f}", loc="right", fontsize=10
        )
    ...
    for ax in axs.values():
        ax.axvspan(gprn.time[0], gprn.time[-1], color="k", alpha=0.05, zorder=-1)
